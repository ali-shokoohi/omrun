...
def Allow_Notify_send(project, from_user, allow_list):
    try:
        subtitle= "پروژه ای جدید ایجاد شده است."
        info = f"پروژه ای جدید به اسم {project.name} و شناسه ی {project.id} ایجاد شده است. شما هم به لیست افراد مجاز اضافه شده اید."
        kind = "پروژه جدید"
        notify = Notifications.objects.create(from_user=from_user, subtitle=subtitle, info=info,
            kind=kind, intent="projects")
        for allow in allow_list:
            email = allow["mation"]
            user = User.objects.get(email=email)
            AllowPersons.objects.create(user=user, project=project)
            NotiPerson.objects.create(notify=notify, person=user)
        return True
